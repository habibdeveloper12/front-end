...
const MyOrder = () => {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/api/v1/order/myPayment/${user?.email}   `)
      .then((res) => res.json())
      .then((data) => {
        console.log(data)
        if (data.success) {
  ...
  return (
    ...
    <div class="row my-5">
    <h3 class="fs-4 mb-3">My Payment Information</h3>
    <div class="col ">
        ...
        <tbody>
          {/* {myOrder?.map((order) => {
            return order?.orderItems?.map((myOrders) => ( */}
              <tr key={myOrder?._id}>
                <th scope="row">{myOrder?.name}</th>
                <td>{myOrder?.status}</td>
                <td>{myOrder?.productId}</td>
                <td>{myOrder?.paidPrice} USD</td>
                ...
                <td>
                  ...
                  <span onClick={() => deleteHenedler(myOrder?._id)} className="delete-btn">
                    <AiFillDelete />
                  </span>
                </td>
              </tr>
  ...
  :
        <div data-aos="flip-left"
          ...
          data-aos-duration="2000" className=" card p-5 w-50 mx-auto">
